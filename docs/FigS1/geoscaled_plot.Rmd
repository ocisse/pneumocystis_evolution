...
t1 <- read.beast("cluster-1_DNA.MCC.tree")
#t <- read.nexus("all_cluster_1.MCC.tree")
todrop <- c("PcarSe6", "PcCcin")
t <- drop.tip(t1, todrop)

t$root.time <- t$height[1]
#t$root.time <- max(dist.nodes(t))

#log_data1 <- read.table("bearsDivtime_FBD.1.log",header=T)
#log_data2 <- read.table("bearsDivtime_FBD.2.log",header=T)
log_data1 <- read.table("run2.log", header = T)
log_data2 <- read.table("../replicat_3/run2.log", header = T)

#nMCMC <- length(log_data1$originFBD)-1
nMCMC <- length(log_data2$posterior)-1
burnin <- 0.2
id1 <- as.integer(nMCMC*burnin)+1
id2 <- nMCMC+1
#comb_origin_data <- c(log_data1$originFBD[id1:id2],log_data2$originFBD[id1:id2])
comb_origin_data <- c(log_data1$posterior[id1:id2],log_data2$posterior[id1:id2])
#comb_origin_data <- c(log_data1$prior[id1:id2],log_data2$prior[id1:id2])
...
pdf("geoscaled_plot.pdf", width = 10, height = 5, pointsize = 8) 
#png("geoscaled_plot1rev.png",
...
#geoscalePhylo(tree=ladderize(t,right=FALSE), boxes="Age", cex.tip=1,cex.age=1,
#				cex.ts=0.7,label.offset=0,x.lim=c(500,x_max), lwd=1.5)

geoscalePhylo(tree=ladderize(t,right=FALSE), boxes="Period", cex.tip=1,cex.age=1,cex.ts=0.9,label.offset=0,lwd=1.5,        
              quat.rm=TRUE,
              tick.scale=50,
               units = c("Era","Period"))
